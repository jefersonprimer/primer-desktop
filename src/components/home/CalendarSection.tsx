    ...
    if (!isCalendarConnected && !isAuthLoading && !isLoading) {
        return (
            <motion.div 
                ...
                className={`group relative overflow-hidden flex ${compact ? 'flex-col items-start gap-4' : 'flex-row items-center justify-between'} p-6 rounded-3xl border border-white/10 backdrop-blur-xl bg-gradient-to-br from-white/[0.03] to-white/[0.01] h-full shadow-2xl shadow-black/20`}
            >
                {/* Background Glow */}
                ...
                >
                    ...
                    Connect
                    <ArrowRightIcon size={16} className="opacity-60"/>
                </motion.button>
            </motion.div>
        );
    }
